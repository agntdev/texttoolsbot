# fix-3f9834ec347d854d — spec.md requires setMyCommands at startup — never called

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TXTBOT

`docs/spec.md:70-71` states: "Commands: register via bot.api.setMyCommands with descriptions for each command." This is also repeated at line 100-101: "Register commands (setMyCommands) with short descriptions at startup."

No code in the entire repository calls `setMyCommands`. As a result, the bot's commands won't appear in Telegram's native command menu UI (the '/' picker). This is a spec violation that degrades the user experience described in the spec.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3f9834ec347d854d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3f9834ec347d854d.json` (and any new command `tests/commands/fix-3f9834ec347d854d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3f9834ec347d854d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3f9834ec347d854d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
