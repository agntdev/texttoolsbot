# fix-91957160f2b7d2af — Env var name mismatch: spec says TELEGRAM_BOT_TOKEN, code uses BOT_TOKEN

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TXTBOT

`docs/spec.md:13` and `docs/spec.md:88` both reference the environment variable `TELEGRAM_BOT_TOKEN`. However:
- `src/index.ts:4` reads `process.env.BOT_TOKEN`
- `AGENTS.md` documents the requirement as `BOT_TOKEN`
- `.env.example` contains `BOT_TOKEN=`

The spec and README are misleading about the required env var name. Anyone following the spec literally would set the wrong variable and the bot would fail to start.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-91957160f2b7d2af.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-91957160f2b7d2af.json` (and any new command `tests/commands/fix-91957160f2b7d2af.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-91957160f2b7d2af.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-91957160f2b7d2af.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
